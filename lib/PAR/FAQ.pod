...
# $Revision: #2 $ $Change: 9517 $ $DateTime: 2003/12/31 14:04:33 $ vim: expandtab shiftwidth=4
...
distribution installations with the default perl package.  To fix this problem,
create a symbolic link from libperl.so.5.6.1 to libperl.so in F</usr/lib>:
...
[Microangelo L<http://www.microangelo.us/> and use that to create your .ico
file. The latest Netpbm tools at L<http://netpbm.sourceforge.net/> has
...
As pointed out by Alan Stewart, zip adds a directory entry for the new directory, and it causes
the PAR executable to fail. 
Just use : 
...
executable into a .zip extension first.

=head2 Can PAR completly hide my source code?

Not completely, but possible to a degree.  Starting from version 0.76, PAR
supports an input filter mechanism (see L<PAR::Filter>), which can be used to
implement source obfuscators (or even product activation schemes).
...
offer (quoted with permission from RandalSchwartz):
